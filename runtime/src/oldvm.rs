    ...
    pub fn run(&mut self) -> ProgramReturn {
        loop {
            ...
            match instr {
                ...
                Instructions::Sweep => todo!(),
                Instructions::SweepShrink => todo!(),
            }
        }
    }
...
impl Memory {
    pub fn read_ptr(&self, ptr: Value) -> Result<Value, Errors> {
        match ptr {
            ...
            Value::CharPtr { obj, offset } => {
                ...
                Ok(Value::Char(
                    string.data.chars().nth(offset as usize).unwrap(),
                ))
            }
            Value::UserdataPtr(_) => todo!(),
        ...
        match ptr {
            ...
            Value::CharPtr { obj, offset } => {
                if let Value::Char(c) = value {
                    let offset = {
                        let mut n = 0;
                        for c in self.strings.data[obj as usize].data.chars() {
                    ...
                    let (s1, s2) = self.strings.data[obj as usize].data.split_at(offset);
                    let s2 = &s2[1..];
                    self.strings.data[obj as usize].data = format!("{}{}{}", s1, c, s2);
                    Ok(())
                } else {
    ...
    pub fn mark_obj(&mut self, obj: usize) {
        self.heap.mark(obj as u64);
        for i in 0..self.heap.objects[obj].data.len() {
            match self.heap.objects[obj].data[i] {
                Value::ObjectPtr(id) => {
                    self.heap.mark(id);
                }
                Value::HeapPtr { obj, .. } => {
    ...
    pub fn mark_obj_idx(&mut self, obj: u64, idx: usize) {
        self.heap.mark(obj);
        let value = self.heap.objects[idx as usize].data[idx];
        match value {
            Value::ObjectPtr(id) => {
                self.heap.mark(id);
            }
    ...
    pub fn mark_str(&mut self, idx: usize) {
        self.strings.mark(idx as u64);
    }
}
...
impl Allocator<Object> for Heap {
    ...
    fn deallocate(&mut self, index: u64) {
        ...
        self.objects[index as usize].visible = false;
    }
...
impl StringObject {
    pub fn new(data: String) -> StringObject {
        StringObject {
            data,
            visible: true,
        }
...
impl Allocator<StringObject> for Strings {
    ...
    fn allocate(&mut self, obj: StringObject) -> u64 {
        match self.freed.pop() {
            Some(id) => {
    ...
    fn deallocate(&mut self, index: u64) {
        self.freed.push(index);
        self.data[index as usize].data.clear();
    }
    ...
    /// for reuse.
    fn shrink(&mut self, index: u64) {
        self.data[index as usize].data.shrink_to_fit();
    }
    ...
    fn realloc(&mut self, index: u64, size: u64) {
        self.data[index as usize].data.reserve(size as usize);
    }
    ...
    fn shrink_all(&mut self) {
        ...
        // find first object that is not freed from the end
        let mut max = 0;
        for (i, obj) in self.data.iter().enumerate().rev() {
            if obj.visible {
                max = i + 1;
                break;
            }
...
pub struct UDHeap {
    pub data: Vec<Box<dyn Userdata>>,
    pub marked: Vec<u64>,
    pub freed: Vec<u64>,
}

pub trait Userdata {
    /// Label for the userdata
    fn label(&self) -> &'static str;
    /// Unique identifier for the library that created the userdata
    fn lib_id(&self) -> u64;
    /// Unique identifier for the userdata
    fn id(&self) -> u64;

    /// Describes how should the userdata be garbage collected
    fn gc_method(&self);
}
...
pub enum Instructions {
    ...
    /// sweep | Sweeps the heap
    Sweep,
    /// sweep_shrink | Sweeps and shrinks the heap
    SweepShrink,
